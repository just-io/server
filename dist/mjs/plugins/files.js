...
export function getMimeTypeByFileExtension(filename) {
    const ext = filename.match(/\.([^.]+)$/)?.[1] ?? '';
    return MIME_TYPES_MAP[ext] ?? UNKNOWN_MIME_TYPE;
}
export function makeLocalFileStorage(pathToDirectory) {
    ...
    return {
        ...
        cleanup: (locations) => {
            return locations.reduce((promise, location) => {
                return promise.then(() => fs.promises.unlink(location));
            }, Promise.resolve());
        },
    };
}
